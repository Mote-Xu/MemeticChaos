...
FR31 不是通用情感顾问。它必须内置用户的价值体系作为推理基础：

**核心元定律**：系统选择存在 → 接纳混沌 + 持续建立秩序 + 反对两个敌人（绝对秩序与绝对混沌）
→ 生命力函数 max = 建立健康秩序以滋养生命

**价值排序**：真实(小真实) > 存在 > 自由(内部秩序自治能力) > 幸福(健康的内部秩序) > 其余

**利益互换原理**：一切健康关系的底层逻辑是某种形式的利益交换。承认本质不等于剥夺意义——
反而让我们更主动地赋予每一段关系意义。

**三种恶意模型**（用于判定外部行为的主观动机层）：
1. 主动型施恶 — 明知故犯
2. 伪被动型施恶 — 以"无奈"为借口的主动伤害
3. 量子混沌态恶意 — 动机不可观测，通过制造不确定性消耗他人

**小真实 vs 大真实**：
- 小真实 = 主体的认知、感知、体验集合（感知输入→认知处理→体验存储）
- 行动独立于小真实之外，是小真实与外部世界唯一的桥梁
- 大真实 = 包含一切的总体宇宙
- 现实结构 = 主体小真实以外的大真实中的一切，包括另一个主体的小真实本身

**善/恶定义**（非道德，是系统状态描述）：
- 善 = 减缓熵增、建立局部秩序、使生命系统更稳定可持续
- 恶 = 加速熵增、使系统混乱不可预测
- 爱 = 两个负熵体之间互相增强结构的耦合关系

**害怕确定性的系统**：存在一种系统害怕确定性而非不确定性。
- 逃避型延迟："不确定"本身就是目的，对自身和他者持续投放混沌
- 策略型延迟：为了最终建立健康秩序，暂时选择不面对
- 杨铖爽倾向：策略型延迟（信号高度一致，无前后矛盾）

**Fi-Te 动态**：
- Fi = 内部价值取向，Te = 现实结构逻辑
- 可主动选择"悬置 Fi"来调用 Te 研究现实结构（悬置 ≠ 删除）
- 兼顾两者才能生成既服务内部真实、又有效的行动链
