...
7. **Scraper v2.0 已建, 采集端+月度聚合已通, 下游未接**：227 v1.0 文件已回溯 embedding (2026-07-09), `monthly_semantic_state.json` 含 2026-06/07 真实语义状态, 但月度语义→x(t)→分析链的最后一段仍未接。
8. **信号源是叙事化反应，非现实结构**：采集的数据测量的是人群对现实结构的叙事化反应。叙事化倾向本身是要测量的对象，不能预设。
9. **预设2(时不变生成机制)在月度数据上不可检验 (2026-07-09)**：时段剪切审计 VERDICT=UNDERPOWERED_CANNOT_REJECT。月度分辨率下协方差探针检验力≈0。**所有基于全127月一次性拟合的结论都隐含"同一系统"前提, 该前提无法证伪也无法坐实。** 这是 epistemic 硬边界, 非方法缺陷。
...
| NFR7 | Evidence Ledger: 所有结论带等级(E0-E4)+假设依赖列, 假设倒了只失效挂靠的高级条目 | 🔧 建设中 |
| NFR8 | Competing Explanatory Layer: 机制假说竞争并存, 不给单一图景抢占唯一答案位 | 🔧 原则确立 |

## 下一步

| 优先级 | 任务 | 说明 |
|:--:|------|------|
| **P0** | **① Evidence Ledger** | E0-E4 + Assumption 列, 现有结论全部分级归档 (方法论基础设施) |
| **P0** | **② 数据管道闭环** | monthly_semantic_state 三标量(cov_trace/各向异性/漂移) 缝进 2025-12 后分析链 |
| P0 | MS-AR Phase 2 | R2 内部 p(x\|R2, z1) 势阱形变建模 |
| P0 | FR31 对接 Stella | engine.py → 企微自动回复 |
| P1 | 低维 TVP-AR 时不变性检验 | 不切段, 标量/低维才有功率; 独立验证分辨率墙 |
