...
## FR19 当前局限 (2026-07-04)

内部叙事层本就是为混沌属性而设的——"不能纯数学建模"那部分。
当前实现还没兑现这个设计意图：

| # | 当前状态 | 本应是什么 |
|---|---------|----------|
| 1 | 约束场是静态标签（每个梗一个向量） | 约束场应随时间变化（同一梗在爆发期 vs 消退期不同） |
| 2 | 内部层用 5 维约束向量 | 内部层应该用叙事结构（传播阶段/变异/漂移） |
| 3 | 五类别是人工标注的脚手架，归类基于叙事特征（非随机），
    但类别应允许交叉（一个梗可同时属于多类），不应互斥 | 之前已确认太粗糙 |
| 4 | 混沌轴 = 随机游走 | 这是实证结论，符合"与混沌共存"预期 |
