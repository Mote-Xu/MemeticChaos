...
策略校准。但缺点致命：
1. 不理解用户的认知框架（Fi-Te 动态、价值真空、量子混沌态恶意等）
2. 爹味，自认认知高于用户，无法区分"认知"和"个人选择"
3. 给的建议是通用"泡学"技巧，不与用户自身的理论体系对接
...
"表白不符合现实结构"来自 Layer 1 的宏观认知——但要不要在当下推进、
怎么推进、用什么语气——这些只能在 Layer 2 和 Layer 3 上决定。

### 核心需求

1. **两层知识底座**：
   - 宏观：MemeticChaos 发现的集体混沌规律（约束场、秩序形态、信号-动机分离）
   - 微观：用户自己的理论框架（Fi-Te 祛魅、量子混沌态恶意、喜欢浓度最佳区间、
     小真实 vs 大真实、主动表白违背现实结构等）
...
- **不变成爹味教练**。不假装比你更了解你自己或对方。
- **忠实于你的框架**。用你的概念工具（Fi-Te、混沌态、约束场）做推理，不译为
  通用心理学词汇。
- **如实标注不确定性**。不知道对方的动机就是不知道，不假装能读心。
