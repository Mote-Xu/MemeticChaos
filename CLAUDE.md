...
- `data/processed/stage_occupancy.json` — 127 月 × 5 阶段占比
- `data/processed/level1_hard_facts.json` — ★ Level 1 硬事实: Stage/Mutation/Institutionalized/SemanticDrift 月度序列
- `data/processed/representation_state.json` — ★ Level 2: 10 维 Narrative State x(t) + H1 验证结果
...
│   ├── narrative_hard_facts.py         # ★ Level 1 硬事实提取 (P0, 2026-07-04)
...
├── models/
│   ├── order_form_predictor.py        # 两层预测模型 (FR19 v0.2, v4.0 已退役)
...
## FR19: Narrative Dynamics (v4.0 重构中)
...
- **H1b**：该表示具有动力学连续性（t 期对 t+1 期有预测力）
- **H1c**：未来演化主要由当前状态决定
...
x(t+1) = F(x(t), u(t), y(t))
```
x = Narrative State（由表示学习决定维度），u = External Field（扰动），y = Attention（反馈）

### 三层渐进式特征工程

- **Level 1**：仅提取硬事实（Stage / Mutation_Occurred / Institutionalized / Semantic_Drift_Distance）
- **Level 2**：Representation Learning（算法由数据决定，不提前指定）
- **Level 3**：训练完成后事后解释特征载荷

### Narrative 是动态图

不是向量——有节点（梗）和有向边（变异/竞争/替代/融合）。
Schema 现支持边定义，工程上用邻接矩阵主特征向量作为状态表示。

### 关键决策

1. 杀混沌轴。废除人工编的权重。
2. 注意力是观测 + 反馈闭环，不是纯摄像头。
3. AlphaGo 原则深入特征工程：数据→表示→解释，不反过来。
4. 不提前指定 Autoencoder/维度/线性形式——由验证结果决定。

详见 `DESIGN_DISCUSSION.md` 和 `FORMALISM.md`。
...
**核心发现**: 月度尺度集体叙事动态由随机漂移主导。验证了"与混沌共存"是实证正确描述。
预测值不是正确的目标——应预测转移概率、结构属性、相变。
