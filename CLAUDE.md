...
| 优先级 | 任务 |
|:--:|------|
| **★主轴** | **历史高分辨率重建 (人类终审 2026-07-12): 往回挖高分辨率、广覆盖历史 (2015-2025), 破月度分辨率墙。★侦察已完成 + 观测算子重构定稿★ (两轮外部AI+人类审计): (a) Google 在华被墙 → Trends geo=CN 测的不是大众, 是"搜索被Google归属到CN"的接入定义/成分未知框; H侧127月结论重标 [Operator: GoogleTrends-CN]。(b) 禁止社会范畴投射(不许精英/大众/成熟/下沉), Population 只按接入机制定义+已知偏斜+其余unknown。(c) 对象=多接入框+跨框不变结构+框间耦合/脱耦, 不假设一个latent X 也不假设两层。修订分档: Trends降级 / 百度日级Dump侦察(走现成不自爬) / 国内新闻Dump(GDELT退成控制变量) / Wayback2019-2022+等密度重采样(archive intensity进模型) / 跨框Nyblom对照(脚本现成) / 建Observation Operator Ledger。详见 PROJECT_STATUS §7** |
