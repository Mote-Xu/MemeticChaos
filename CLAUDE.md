...
- Regime 离散性审计 — gap statistic→**k=1** (BIC 只搜 k≥3), GMM(4) 全簇自举 Jaccard 0.20-0.56 (**R2 亦仅 0.48**), 仅 PC1 label-free 多峰。RQA 的 R2 分离可由**慢漂移驻留**解释 → **"4 相区"降级为操作切分, R2 as 分离盆地不获支持 (as 高驻留区存活)**; 127×10 稀疏 UNDERPOWERED 不坐实连续。承重: regime-discretization holds→**suspect** (2026-07-12, `regime_discreteness.py`)
