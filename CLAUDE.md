...
内部叙事层本就是为"不能纯数学建模的混沌属性"而设的，但当前实现还没兑现：

- **约束场是静态标签**：每个梗只有一个向量，不随时间变化。
  同一个梗在爆发期和消退期用同一个约束值。
- **内部层被偷换概念**：内部叙事层本应是 87 条叙事的结构信息（传播阶段、变异类型、
  语义漂移），但当前实现把它压缩成了 5 维约束向量。约束场 R²=0.52 只能说明
  约束场可从外部特征预测，与"叙事层是否激活"无关。
- **混沌轴作为随机游走**是实证结论，不是 bug。"与混沌共存"一直在说这个——
  个别月不可预测，但秩序形态有结构。
- **五类别是脚手架**：归类基于叙事特征有逻辑支撑，但应允许多标签
  （一个梗可属于多个类别，当前互斥分类有信息损失）。最终要被数据驱动聚类替代。问题
